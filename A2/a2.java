...
public class Assignment2 {
    ...
    public boolean assignGrader(int groupID, String grader) {
        ...
        try {
            ...
            // Check if resultset is not an empty table
            if (rs.next()) {
 ...
 //               System.out.println("Invalid group ID!");
    ...
    public boolean createGroups(int assignmentToGroup, int otherAssignment,
        ...
        try {
            ...
            // Prepare the appropriate database query
            queryString = "DROP VIEW IF EXISTS all_students CASCADE";
            ps = connection.prepareStatement(queryString);
            ps.executeUpdate();
            
            
            // Step 1 - check if both assignment IDs are in the assignment table
            ...
            ps.setInt(2, otherAssignment);
            rs = ps.executeQuery();

            // Check if resultset is an empty table
            if (!rs.next()) {
		//System.out.println("Invalid assignment IDs!");   
                return false;
		...
		//System.out.println("Invalid assignment IDs!");
            ...
            // Step 2 - check if any group has already been defined for this assignment
            ...
            // Check if resultset is not an empty table
            if (rs.next()) {            
                //System.out.println("Groups have been defined!");
                return false;
            } 
            
            
            // Step 3 - get group_max for this assignment 
            ...
            // Step 4 - get the existing largest group ID 

            // Prepare the appropriate database query
            queryString = "SELECT max(group_id) " +
                "FROM AssignmentGroup";
            ps = connection.prepareStatement(queryString);
            // Execute the PreparedStatement
            rs = ps.executeQuery();
            // get max group ID from query result
            int group_id_max;
            if (rs.next()) {
            	group_id_max = rs.getInt(1);
            } else {
            	group_id_max = 0;
            }
            
            
            // Step 5 - manipulate the value of the SERIAL's counter           
            
            // Prepare the appropriate database query
            queryString = "SELECT setval('AssignmentGroup_group_id_seq', ?)";            
            ps = connection.prepareStatement(queryString);
            ps.setInt(1, group_id_max);
            rs = ps.executeQuery();
            
            
            // Step 6 - select all the students and check if there are any students

            // Prepare the appropriate database query
            queryString = "CREATE VIEW all_students AS " +
                "(SELECT username " +
                "FROM MarkusUser " +
                "WHERE type = 'student')";
            ps = connection.prepareStatement(queryString);
            ...
            // Prepare the appropriate database query
            queryString = "SELECT * FROM all_students";
            ps = connection.prepareStatement(queryString);
            rs = ps.executeQuery();
            ...
            // Step 7 - list all students in the right order

            // Prepare the appropriate database query
            queryString = "CREATE TABLE student_mark " + 
		"(username VARCHAR(25), mark real)";
            ps = connection.prepareStatement(queryString);
            // Execute the PreparedStatement
            ps.executeUpdate();

            // Prepare the appropriate database query
            queryString = "INSERT INTO student_mark " +
		"(SELECT distinct username, mark " +
                "FROM Result NATURAL RIGHT JOIN Membership " +
                "NATURAL RIGHT JOIN AssignmentGroup " +
                "WHERE assignment_id = ? " +
            ...
            // Prepare the appropriate database query
            queryString = "SELECT distinct username, mark " +
                "FROM student_mark NATURAL RIGHT JOIN all_students " +
                "ORDER BY mark DESC NULLS LAST, username ASC";
            ps = connection.prepareStatement(queryString);
            // Execute the PreparedStatement
            ResultSet rs_final = ps.executeQuery();
            
            
            // Step 8 - create groups and add members           
            
            int member_counter = group_max;
            
            while (rs_final.next()) {
            
		String username = rs_final.getString("username");		
		
		// Step 8.1 - create a new group if there's no space for new member
		
		if (member_counter == group_max){
		    
		    group_id_max++;
		    String group_id_String = Integer.toString(group_id_max);
		    String repo = repoPrefix + "/group_" + group_id_String;
		    
		    // Prepare the appropriate database query
		    queryString = "INSERT INTO AssignmentGroup(assignment_id, repo) " +		    
			"VALUES (?, ?)";
		    ps = connection.prepareStatement(queryString);
		    // Insert that string into the PreparedStatement and execute it.
		    ps.setInt(1, assignmentToGroup);
		...
		// Step 8.2 - add a group member
		
		this.recordMember(assignmentToGroup, group_id_max, username);
		member_counter++;
		
            }            
            
            // Drop table used for intermediate step
            queryString = "DROP TABLE IF EXISTS student_mark CASCADE";
            ps = connection.prepareStatement(queryString);
            ps.executeUpdate();
            ...
            System.out.println("Failed to create groups.");
            e.printStackTrace();
            return false;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            boolean conn;
            // check connection
            conn = a2.connectDB("jdbc:postgresql://localhost:5432/csc343h-yanggan", "yanggan", "");
...
*/
            ...
            //test n: insert successful 
            result = a2.createGroups(1003, 1000, "repo_url");
            System.out.println("f3: test 1: insert successful -" + result);
            //check AssignmentGroup table
            queryString_main = "SELECT * " +
                "FROM AssignmentGroup";
            ps_main = a2.connection.prepareStatement(queryString_main);
            rs_main = ps_main.executeQuery();
            System.out.println("AssignmentGroup table--------------------------");
            System.out.println("group_id" + "\t" + "assignment_id");
            while (rs_main.next()) {
                group_id_result = rs_main.getInt("group_id");
                assignment_id_result = rs_main.getInt("assignment_id");
                String reop = rs_main.getString("repo");
                System.out.println(group_id_result + "\t" + assignment_id_result + "\t" + reop);
            }
